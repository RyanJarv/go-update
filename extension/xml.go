...
func (updateResponse *WebStoreUpdateResponse) MarshalXML(e *xml.Encoder, start xml.StartElement) error {
	...
	url := "https://s3.amazonaws.com/brave-extensions/release/" + updateResponse.ID + "/" + updateResponse.Title
	app := App{
		AppID:  updateResponse.ID,
		Status: "ok",
		UpdateCheck: UpdateCheck{
			Status:   "ok",
			SHA256:   updateResponse.SHA256,
			Version:  updateResponse.Version,
			Codebase: url,
		},
	}
	if len(updateResponse.ID) > 0 {
		response.App = &app
	}
